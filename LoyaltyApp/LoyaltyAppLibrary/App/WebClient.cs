...
namespace LoyaltyAppLibrary.App
{
    public abstract class WebClient
    {
        ...
        TokenInfo _userTokenInfo;
        TokenInfo _appTokenInfo;
        public async Task<bool> LoadTokenFileAsync()
        {
            try
            {
                ...
                {
                    byte[] result = Client.GetInstance().FileClient.ReadAllBytes(Client.GetInstance().FileClient.GetDataLoction(), Configuration.tokenInfoFile);
                    return result;
                });
        ...
        public async Task<bool> SaveTokenFileAsync()
        {
            ...
            try
            {
                bool wResult = await Task.Run(() =>
                {
                    ...
                    bool result = Client.GetInstance().FileClient.WriteAllBytes(cipherText, Client.GetInstance().FileClient.GetDataLoction(), Configuration.tokenInfoFile);
                    return true;
                });
                return wResult;
            }
        ...
        async Task<TokenInfo> GetTokenAsync(TokenType tokenType)
        {
        ...
        RestRequest CreateRequest(TokenInfo tokenInfo, string requestUrl, HttpMethod method, params Tuple<string, string>[] parameters)
        {
        ...
        RestRequest CreateRequest(TokenInfo tokenInfo, string requestUrl, HttpMethod method, Object obj, params Tuple<string, string>[] parameters)
        {
            var request = new RestRequest(requestUrl, method);
            request.AddParameter("Authorization", string.Format("Bearer {0}", tokenInfo.AccessToken), ParameterType.HttpHeader);
            request.AddHeader("Accept", "application/json");
        ...
        RestRequest CreateRequest(TokenInfo tokenInfo, string requestUrl, HttpMethod method, object obj)
        {
